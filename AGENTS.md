...
候補への短い修正意見はBaseの`review_comment`に、長い意見は候補本文の`## ユーザーコメント`に書く。ユーザーが「候補コメントを反映して」などと依頼した場合は、候補の主張・下書き・根拠・反例を修正し、元コメントと対応内容を`## コメント反映履歴`へ残す。反映後は未処理コメントを空にする。`decision`、`apply_status`、元ノート、正式ノートは変更しない。
...
また、次のような依頼は `Society` 単体スコープとして扱ってよい:
...
また、次のような依頼は `reading-note` 全体スコープとして扱ってよい:
...
また、次のような依頼は `Life / Society / Learning` を主対象にしたコアテーマ発見として扱ってよい:
...
また、次のような依頼は各単一タグや交差タグのスコープとして扱ってよい:
...
Codex はコマンド名を要求せず、標準サイクルを実行してよい。

`Life / Society / Learning` のコアテーマ発見サイクル:
